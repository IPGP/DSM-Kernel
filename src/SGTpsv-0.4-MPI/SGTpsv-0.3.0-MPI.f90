...
program  SGTforPinv
  ...
  !-------------------------<< input matrix >>----------------------------------
  !include 'mpif.h'  
  character(120) :: outputDir, psvmodel, modelname 
  ...
  if(my_rank.eq.0) then 
     ...
     call pinput(outputDir,psvmodel,modelname,tlen,rmin_,rmax_,rdelta_,r0min,r0max,r0delta,thetamin,thetamax,thetadelta,imin,imax,rsgtswitch)
...
!  print *, "nickel: bcast", my_rank
  ...
  do i=imin,imax            ! f-loop start
     ...
     if((i.ne.0).and.((mod(imax-my_rank-i,2*nproc).eq.0).or.(mod(imax+my_rank+1-i,2*nproc).eq.0))) then
           ...
           do m=-2,2        ! m-loop start
              ...
              if ( iabs(m).le.iabs(l) ) then
                 ...
                 if ( l.eq.0 ) then ! l-branch for calu (l=0) 
                    ...
                    if((m.eq.0).and.(rsgtswitch.eq.1)) then  ! back propagated
                       do icomp = 1,2
                          g0 = cmplx(0.d0)
                          ...
                          do ir_=1,r_n
                             ...
                             do itheta = 1, theta_n
                                ...
                                call calup0(g0tmp(1),dvecdp0(1:3,m,itheta),udp(1:3))
                                call locallyCartesianDerivatives(u(1:3),udr(1:3),udt(1:3),udp(1:3),uder(1:3,1:3),r_(ir_),theta(itheta)/180.d0*pi)
                                call udertorsgt(icomp,uder(1:3,1:3),rsgt(1:2,ir_,itheta))
                             enddo
                          enddo
                       enddo
                    endif
        ...
        do ir_ = 1,r_n
           do ir0 = 1,r0_n
              write(coutfile, '(I5,".",I5,".",I5,".TSGT_PSV")') int(r0(ir0)*10.d0),int(r_(ir_)*10.d0),i
              do j = 1,23
           ...
           enddo
           if(rsgtswitch) then
              write(coutfile, '(I5,".",I5,".RSGT_PSV")') int(r_(ir_)*10.d0),i
              do j = 1,17
                 if (coutfile(j:j).eq.' ')coutfile(j:j) = '0'
              enddo
              coutfile = trim(modelname)//"."//coutfile
              ...
              open(1,file=coutfile,status='unknown',form='unformatted', &
                   access = 'direct', recl=2*2*kind(0e0)*theta_n)
              rsgtsngl(1:2,1:theta_n) = rsgt(1:2,ir_,1:theta_n)
              write(1,rec=1)rsgtsngl(1:2,1:theta_n)
              close(1)                    
           endif
        enddo
  ...
  if(my_rank.eq.0) then   
     ...
     do i = 0, imax
        ...
        elseif(i.eq.0) then
           ...
           do ir_ = 1,r_n
              do ir0 = 1,r0_n
                 write(coutfile, '(I5,".",I5,".TSGT_PSV")') int(r0(ir0)*10.d0),int(r_(ir_)*10.d0)
                 do j = 1,23
                    if (coutfile(j:j).eq.' ')coutfile(j:j) = '0'
                 enddo
                 coutfile = trim(modelname)//"."//coutfile
                 ...
                 open(1,file=coutfile,status='unknown',form='unformatted', &
                      access = 'direct', recl=2*num_tsgt*kind(0e0)*theta_n)
                 !tsgtsngl(1:num_tsgt,1:theta_n) = tsgt(1:num_tsgt,ir_,1:theta_n,ir0)
                 tsgtsngl(1:num_tsgt,1:theta_n) = cmplx(0.e0)
                 read(1,rec=i+1)tsgtsngl(1:num_tsgt,1:theta_n)                 
                 close(1)          
              enddo
              if(rsgtswitch) then
                 write(coutfile, '(I5,".RSGT_PSV")') int(r_(ir_)*10.d0)
                 do j = 1,17
                    if (coutfile(j:j).eq.' ')coutfile(j:j) = '0'
                 enddo
                 coutfile = trim(modelname)//"."//coutfile
                 ...
                 open(1,file=coutfile,status='unknown',form='unformatted', &
                      access = 'direct', recl=2*2*kind(0e0)*theta_n)
                 rsgtsngl(1:2,1:theta_n) = rsgt(1:2,ir_,1:theta_n)
                 read(1,rec=i+1)rsgtsngl(1:2,1:theta_n)
                 close(1)                    
              endif
           enddo
        else
           do ir_ = 1,r_n
              do ir0 = 1,r0_n
                 ...
                 open(1,file=coutfile,status='unknown',form='unformatted', &
                      access = 'direct', recl=2*num_tsgt*kind(0e0)*theta_n)
                 tsgtsngl(1:num_tsgt,1:theta_n) = tsgt(1:num_tsgt,ir_,1:theta_n,ir0)
                 read(1,rec=1)tsgtsngl(1:num_tsgt,1:theta_n)                 
                 close(1)         
              ...
              if(rsgtswitch) then
                 ...
                 open(1,file=coutfile,status='unknown',form='unformatted', &
                      access = 'direct', recl=2*2*kind(0e0)*theta_n)
                 rsgtsngl(1:2,1:theta_n) = rsgt(1:2,ir_,1:theta_n)
                 read(1,rec=1)rsgtsngl(1:2,1:theta_n)
                 close(1)
